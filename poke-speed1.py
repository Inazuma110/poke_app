...
#どんな素早さ種族値を抜けるかを記述
def tellPokeSp(std,sem,max,sp,pokemon,mine):
	fmt = """
	{pokemon}族の{sp}の素早さは、{mine} で、
	{std}族の無振り、{sem}族の準速、{max}族の最速ポケモンを抜いています。
	"""
	print(fmt.format(pokemon = pokemon,sp = sp2[sp],mine = mine,std = std, sem = sem,max = max))
...
while True:
	...
	tellPokeSp(N1,N2,N3,sp,pokemon,mySpeed)
